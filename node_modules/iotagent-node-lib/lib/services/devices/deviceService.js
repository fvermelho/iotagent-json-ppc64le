...
/**
 * Call all device notification listeners with a specific event and device
 * @param {event} event The event associated to the device.
 * @param {Object} deviceObj The object being notified.
 */
function notifyListeners(event, deviceObj, callback) {
    // Call all device notification callbacks
    for (var i = 0; i < deviceNotificationCallbacks.length; i++) {
        deviceNotificationCallbacks[i](event, deviceObj);
    }
    callback();
}
    ...
    function completeRegistrations(error, deviceData) {
        ...
        async.waterfall([
            ...
            if (error) {
                ...
                notifyListeners(events.DEVICE_CREATED, deviceObj, function() { /** empty callback */ });
            }
        });
    }
        ...
        async.waterfall([
            ...
            if (error) {
                ...
                async.waterfall([
                    ...
                    apply(notifyListeners, events.DEVICE_REMOVED, mergedDevice)
                ], callback);
            }
        });
    });
...
function updateRegisterDevice(deviceObj, callback) {
    ...
    async.waterfall([
        ...
        apply(notifyListeners, events.DEVICE_UPDATED)
    ], callback);
}
...
/**
 * Add a new device notification callbacks
 * These callbacks should have two attributes:
 *  - event: what's happening to the device being notified
 *  - device: an object containing the device being notified
 *
 * @param {callback} Function The callback to be added.
 */
function addDeviceNotificationCallback(callback) {
    deviceNotificationCallbacks.push(callback);
}
...
exports.addDeviceNotificationCallback = addDeviceNotificationCallback;
