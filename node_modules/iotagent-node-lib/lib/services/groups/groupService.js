...
/**
 * Call all group notification listeners with a specific event and group
 * @param {event} event The event associated to the device.
 * @param {Object} groupObj The object being notified.
 */
function notifyListeners(event, groupObj, callback) {
    // Call all group notification callbacks
    for (var i = 0; i < groupNotificationCallbacks.length; i++) {
        groupNotificationCallbacks[i](event, groupObj);
    }
...
function createGroup(groupSet, callback) {
    ...
    for (var i = 0; i < groupSet.services.length; i++) {
        insertions.push(async.apply(notifyListeners, events.GROUP_CREATED, groupSet.services[i]))
    }
...
function remove(service, subservice, resource, apikey, callback) {
    ...
    async.waterfall([
        ...
        apply(notifyListeners, events.GROUP_REMOVED)
...
function update(service, subservice, resource, apikey, body, callback) {
    ...
    async.waterfall([
        ...
        apply(notifyListeners, events.GROUP_UPDATED)
    ], handleWithIotaRegistration(callback));
}
...
function addGroupNotificationCallback(callback) {
...
exports.addGroupNotificationCallback = addGroupNotificationCallback;
